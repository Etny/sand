...
impl Window {
    pub fn new(title: &str, world: &particles::World, cell_size: (f32, f32)) -> Self {
        ...
        let size = PhysicalSize::new(
            (world.width() as f32 * cell_size.0).floor() as u32,
            (world.height() as f32 * cell_size.1).floor() as u32,
        );
        ...
        let shape = vec![
            ...
            Vertex {
                position: [1.0, 1.0],
                tex_coords: [world_size.0, 0.0],
            },
            Vertex {
                position: [1.0, -1.0],
                tex_coords: [world_size.0, world_size.1],
            },
        ];
        ...
        let quad_index_buffer = glium::index::IndexBuffer::new(
            &display,
    ...
    pub fn mouse_in_window(&self) -> bool {
        match self.mouse_pos {
            Some(pos) => pos.0 < self.size.0 as f64 && pos.1 < self.size.1 as f64,
            None => false,
        }
    }
    ...
    pub fn run(mut self, mut world: particles::World) {
        ...
        let texture = BufferTexture::persistent(
            &self.display,
            &world.cells().data(),
            ...
            match event {
                glutin::event::Event::WindowEvent { ref event, .. } => match event {
                    ...
                    glutin::event::WindowEvent::Resized(size) => {
                        ...
                        self.cell_size = (
                            size.width as f32 / world.width() as f32,
                            size.height as f32 / world.height() as f32,
                        );
                    }
                        ...
                        if input.state == glutin::event::ElementState::Pressed {
                            match input.virtual_keycode.unwrap() {
                                glutin::event::VirtualKeyCode::Key1 => {
                                    self.select_material(particles::Material::Sand)
                                }
                                glutin::event::VirtualKeyCode::Key2 => {
                                    self.select_material(particles::Material::Water)
                                }
                                glutin::event::VirtualKeyCode::Key3 => {
                                    self.select_material(particles::Material::Wood)
                                }
                                glutin::event::VirtualKeyCode::Key4 => {
                                    self.select_material(particles::Material::Fire)
                                }
                                glutin::event::VirtualKeyCode::Key5 => {
                                    self.select_material(particles::Material::Flames)
                                }
                                glutin::event::VirtualKeyCode::Key6 => {
                                    self.select_material(particles::Material::Oil)
                                }
                                glutin::event::VirtualKeyCode::Key7 => {
                                    self.select_material(particles::Material::Grass)
                                }
                                _ => (),
                            }
                        }
                    ...
                    if now - last_tick >= tick_time {
                        ...
                        if self.mouse_down && self.mouse_in_window() {
                            if let Some(pos) = self.mouse_pos {
                                world.set_cell(
                                    (pos.0 as f32 / self.cell_size.0) as u32,
                                    (pos.1 as f32 / self.cell_size.1) as u32,
